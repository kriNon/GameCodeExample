...
const float UArrowMovementComponent::MIN_TICK_TIME = 1e-6f;

UArrowMovementComponent::UArrowMovementComponent(const FObjectInitializer& ObjectInitializer)
...
void UArrowMovementComponent::PostLoad()
{
	Super::PostLoad();
...
void UArrowMovementComponent::InitializeComponent()
{
	...
	InitialPosition = UpdatedComponent->GetComponentLocation();
}


void UArrowMovementComponent::UpdateTickRegistration()
{
...
void UArrowMovementComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
...
bool UArrowMovementComponent::HandleDeflection(FHitResult& Hit, const FVector& OldVelocity, const uint32 NumBounces, float& SubTickTimeRemaining)
{
...
bool UArrowMovementComponent::HandleSliding(FHitResult& Hit, float& SubTickTimeRemaining)
{
...
void UArrowMovementComponent::SetVelocityInLocalSpace(FVector NewVelocity)
{
	if (UpdatedComponent)
	{
...
FVector UArrowMovementComponent::ComputeVelocity(FVector InitialVelocity, float DeltaTime) const
{
	// v = v0 + a*t
...
FVector UArrowMovementComponent::LimitVelocity(FVector NewVelocity) const
{
	...
	// if(FVector::Distance(InitialPosition, UpdatedComponent->GetComponentLocation()) <= ProjectileNoGravityRange)
	// {
	// 	if (NewVelocity.Size() < ProjectileStabilitySpeed)
	// 	{
	// 		NewVelocity = NewVelocity.GetSafeNormal() * ProjectileStabilitySpeed;
	// 	}
	// }
	// else
	// {
	// 	if (NewVelocity.Size() < ProjectileFallStabilitySpeed)
	// 	{
	// 		NewVelocity = NewVelocity.GetSafeNormal() * ProjectileFallStabilitySpeed;
	// 	}
	// }
...
FVector UArrowMovementComponent::ComputeMoveDelta(const FVector& InVelocity, float DeltaTime) const
{
...
FVector UArrowMovementComponent::ComputeAcceleration(const FVector& InVelocity, float DeltaTime) const
{
	...
	if (ProjectileAcceleration != 0)
	{
		if(FVector::Distance(InitialPosition, UpdatedComponent->GetComponentLocation()) <= ProjectileNoGravityRange)
		{
			if(InVelocity.Size() > ProjectileStabilitySpeed)
			{
				Acceleration += InVelocity.GetSafeNormal() * ProjectileAcceleration;
			}
		}
		else
		{
			if(InVelocity.Size() > ProjectileFallStabilitySpeed)
			{
				Acceleration += InVelocity.GetSafeNormal() * ProjectileFallAcceleration;
			}
		}
	}

	return ConstrainDirectionToPlane(Acceleration);
}

// Allow the projectile to track towards its homing target.
FVector UArrowMovementComponent::ComputeHomingAcceleration(const FVector& InVelocity, float DeltaTime) const
{
...
void UArrowMovementComponent::AddForce(FVector Force)
{
	PendingForce += Force;
}

void UArrowMovementComponent::ClearPendingForce(bool bClearImmediateForce)
{
...
float UArrowMovementComponent::GetGravityZ() const
{
	// TODO: apply buoyancy if in water
	return ShouldArrowApplyGravity() ? ProjectileGravity * ProjectileGravityScale : 0.f;
}

bool UArrowMovementComponent::ShouldArrowApplyGravity() const
{
	if(Velocity.Size() <= ProjectileStabilitySpeed)
	{
...
void UArrowMovementComponent::StopSimulating(const FHitResult& HitResult)
{
...
UArrowMovementComponent::EHandleBlockingHitResult UArrowMovementComponent::HandleBlockingHit(const FHitResult& Hit, float TimeTick, const FVector& MoveDelta, float& SubTickTimeRemaining)
{
...
FVector UArrowMovementComponent::ComputeBounceResult(const FHitResult& Hit, float TimeSlice, const FVector& MoveDelta)
{
...
void UArrowMovementComponent::HandleImpact(const FHitResult& Hit, float TimeSlice, const FVector& MoveDelta)
{
...
bool UArrowMovementComponent::CheckStillInWorld()
{
	if ( !UpdatedComponent )
	{
		return false;
...
bool UArrowMovementComponent::ShouldUseSubStepping() const
{
	return bForceSubStepping || (GetGravityZ() != 0.f) || (bIsHomingProjectile && HomingTargetComponent.IsValid());
}


float UArrowMovementComponent::GetSimulationTimeStep(float RemainingTime, int32 Iterations) const
{
...
void UArrowMovementComponent::SetInterpolatedComponent(USceneComponent* Component)
{
...
USceneComponent* UArrowMovementComponent::GetInterpolatedComponent() const
{
	return InterpolatedComponentPtr.Get();
}

void UArrowMovementComponent::MoveInterpolationTarget(const FVector& NewLocation, const FRotator& NewRotation)
{
	if (!UpdatedComponent)
	{
		return;
...
void UArrowMovementComponent::ResetInterpolation()
{
	if (USceneComponent* InterpComponent = GetInterpolatedComponent())
	{
...
void UArrowMovementComponent::TickInterpolation(float DeltaTime)
{
	if (!bInterpolationComplete)
	{
		if (USceneComponent* InterpComponent = GetInterpolatedComponent())
