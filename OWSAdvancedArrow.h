...
#include "OWSAdvancedArrow.generated.h"
...
struct FRepUTArrowMovement
{
	...
	UPROPERTY()
	    float ProjectileStabilitySpeed;
	
	UPROPERTY()
	    float ProjectileFallAcceleration;
	
	UPROPERTY()
	    float ProjectileFallStabilitySpeed;
	    
	UPROPERTY()
        float ProjectileStraightLineDistance;

	FRepUTArrowMovement()
		: LinearVelocity(ForceInit)
		, Location(ForceInit)
		, Rotation(ForceInit)
	    , ProjectileInitialSpeed(ForceInit)
	    , ProjectileGravity(ForceInit)
	    , ProjectileAcceleration(ForceInit)
	    , ProjectileStabilitySpeed(ForceInit)
	    , ProjectileFallAcceleration(ForceInit)
	    , ProjectileFallStabilitySpeed(ForceInit)
	    , ProjectileStraightLineDistance(ForceInit)
	{}
	...
	bool operator==(const FRepUTArrowMovement& Other) const
	{
...
class OWSPLUGIN_API AOWSAdvancedArrow : public AActor
{
...
protected:	
	...
	UPROPERTY(ReplicatedUsing = OnRep_UTProjReplicatedMovement)
		struct FRepUTArrowMovement UTProjReplicatedMovement;
	...
	UPROPERTY(BlueprintReadWrite, Category = Projectile)
		USphereComponent* PawnCollisionSphere;
	...
	virtual bool CanMatchFake(AOWSAdvancedArrow* InFakeProjectile, const FVector& VelDir) const;

	/** Synchronize replicated projectile with the associated client-side fake projectile */
	virtual void BeginFakeArrowsSynch(AOWSAdvancedArrow* InFakeProjectile);
	...
	UFUNCTION()
	    virtual void OnRep_UTProjMovementValues();
...
public:	
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Projectile)
		UArrowMovementComponent* ArrowMovement;
	
	UPROPERTY(ReplicatedUsing = OnRep_UTProjMovementValues, EditAnywhere, BlueprintReadWrite, Meta = (ExposeOnSpawn = true), Category = Projectile)
	    float ProjectileFirstSpeed;
	...
	*/
	...
	virtual void InitFakeArrow(class AOWSPlayerController* OwningPlayer);
